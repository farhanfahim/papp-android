...
public class PrivateThreadsFragment extends ThreadsFragment {
    ...
    public PrivateThreadsFragment(boolean single) {
        this.single = single;
    }
...
//                }));
    ...
    protected List<Thread> getThreads() {
        ...
        if(single) {
            threads.addAll(ChatSDK.thread().getThreads(ThreadType.Private1to1));
        } else {
            threads.addAll(ChatSDK.thread().getThreads(ThreadType.PrivateGroup));
        }
        return threads;
    }
