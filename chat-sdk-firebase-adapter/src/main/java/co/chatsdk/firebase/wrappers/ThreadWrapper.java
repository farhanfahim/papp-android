...
//            }
    ...
    public Completable deleteThreadPermanently() {
        return Completable.create(e -> {
            FirebasePaths.threadRef(model.getEntityID()).removeValue((databaseError, databaseReference) -> {
                e.onComplete();
            });
    ...
    public Observable<Boolean> hasShareAudioAccess() {
        return Observable.create((ObservableOnSubscribe<Boolean>) e -> {
            User user = ChatSDK.currentUser();
            FirebasePaths.threadUsersRef(model.getEntityID()).child(user.getEntityID()).child(Keys.AudioAccess).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    if(dataSnapshot.getValue() != null) {
                        long hasAccess = (long) dataSnapshot.getValue();
                        e.onNext(hasAccess == 1);
                    } else {
                        e.onNext(false);
                    }
                }
    ...
    public Observable<Boolean> isGroupAdmin() {
        return Observable.create((ObservableOnSubscribe<Boolean>) e -> {
            User user = ChatSDK.currentUser();
            FirebasePaths.threadUsersRef(model.getEntityID(), user.getEntityID()).addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    for(DataSnapshot item : dataSnapshot.getChildren()) {
                        if(item.getKey().equals("status") && item.getValue().equals("owner")) {
                            e.onNext(true);
                        } else {
                            e.onNext(false);
                        }
    ...
    public Completable makeAdminToEarliestPerson(String userId) {

        return Completable.create(e -> {
            Map<String,Object> params = new HashMap<>();
            params.put("status", "owner");
            FirebasePaths.threadUsersRef(model.getEntityID(), userId).updateChildren(params, new DatabaseReference.CompletionListener() {
                @Override
